...
public final class QueryUtils {
    ...
    * Query the USGS dataset and return a list of {@link EarthquakeInfo} objects.
     * following is a method of fetchEarthquakeData to fetch the url and use it in the mainactivity.
    */
         ...
         // If the JSON string is empty or null, then return early.
            ...
            try {
                ...
                for (int i = 0; i < earthquakeArray.length(); i++) {
                    ...
                    EarthquakeInfo earthquake = new EarthquakeInfo(magnitude,locationOffset,primaryLocation, time, url);
    ...
    //Modify the extractEarthquakes() method that handles JSON parsing.
    // I renamed it to extractFeatureFromJson() and to take a JSON response String as input.
    // Instead of hardcoding the extractFeatureFromJson() method to only be able to parse the hardcoded SAMPLE_JSON_RESPONSE String,
    // this method becomes more reusable in different contexts if we accept a String input.
...
//
//        // Create an empty ArrayList that we can start adding earthquakes to
//        ArrayList<EarthquakeInfo> earthQuakesInfo = new ArrayList<>();
//
...
//                // Creating an JSONobjects with the corresponding data from sample JSON.
//                JSONObject jsonObj = new JSONObject(SAMPLE_JSON_RESPONSE);
//                //Getting the JSONArray from the object features with a list of features.
//                JSONArray quakesArray = jsonObj.getJSONArray("features");
//
//                //For each earthquakes in the Earthquakes array
//                // looping of the object at position i
//                for (int i = 0; i < quakesArray.length(); i++) {
//                    //Retrieving of the data and list in properties
//                    JSONObject currentQuakes = quakesArray.getJSONObject(i);
//                    JSONObject properties = currentQuakes.getJSONObject("properties");
//
////                  String magnitude = properties.getString("mag");
//                    // Extract the value for the key called "mag" from JSON using double type
//                    double magnitude = properties.getDouble("mag");
//
////                  String[] place = properties.getString("place").split("(?<=of)");
////                  String coordinate = place[0];
////                  String location = place[0];
//                    // Extract the string for the key called "place" from JSON
//                    String location = properties.getString("place");
//                    String primaryLocation;
//                    String locationOffset;
//                    if (location.contains( LOCATION_SEPARATOR)) {
...
//                    }
//
//                    // Extract the time from JSON and store it using long type
//                    long time = properties.getLong("time");
//
//                    //Extract the url from JSON and use it for intent click to the website from each list
//                    String url = properties.getString("url");
//
//                    //After retrieving the info put the earthquakeInfo into the list of earthQuakesInfo
//                    // Create a new {@link Earthquake} object with the magnitude, location, time,and url from the JSON response.
//                    EarthquakeInfo earthquakeInfo = new EarthquakeInfo(magnitude,locationOffset,primaryLocation,time,url);
//                    //adding each retrieved info from JSON and returned the earthQuakeInfo list.
//                    earthQuakesInfo.add(earthquakeInfo);
//
//                }
//
//                } catch(JSONException e){
